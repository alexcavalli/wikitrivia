defmodule Wikitrivia.QuestionsTest do
  ...
  describe "answer_choices" do
    ...
    def answer_choice_fixture(attrs \\ %{}) do
      {:ok, answer_choice} =
        attrs
        |> Enum.into(@valid_attrs)
        |> Questions.create_answer_choice()

      answer_choice
    end

    test "list_answer_choices/0 returns all answer_choices" do
      answer_choice = answer_choice_fixture()
      assert Questions.list_answer_choices() == [answer_choice]
    end

    test "get_answer_choice!/1 returns the answer_choice with given id" do
      answer_choice = answer_choice_fixture()
      assert Questions.get_answer_choice!(answer_choice.id) == answer_choice
    end

    test "create_answer_choice/1 with valid data creates a answer_choice" do
      assert {:ok, %AnswerChoice{} = answer_choice} = Questions.create_answer_choice(@valid_attrs)
    end

    test "create_answer_choice/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Questions.create_answer_choice(@invalid_attrs)
    end

    test "update_answer_choice/2 with valid data updates the answer_choice" do
      answer_choice = answer_choice_fixture()
      assert {:ok, answer_choice} = Questions.update_answer_choice(answer_choice, @update_attrs)
      assert %AnswerChoice{} = answer_choice
    end

    test "update_answer_choice/2 with invalid data returns error changeset" do
      answer_choice = answer_choice_fixture()
      assert {:error, %Ecto.Changeset{}} = Questions.update_answer_choice(answer_choice, @invalid_attrs)
      assert answer_choice == Questions.get_answer_choice!(answer_choice.id)
    end

    test "delete_answer_choice/1 deletes the answer_choice" do
      answer_choice = answer_choice_fixture()
      assert {:ok, %AnswerChoice{}} = Questions.delete_answer_choice(answer_choice)
      assert_raise Ecto.NoResultsError, fn -> Questions.get_answer_choice!(answer_choice.id) end
    end

    test "change_answer_choice/1 returns a answer_choice changeset" do
      answer_choice = answer_choice_fixture()
      assert %Ecto.Changeset{} = Questions.change_answer_choice(answer_choice)
    end
  end
end
